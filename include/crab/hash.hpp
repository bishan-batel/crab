...
namespace crab {
  ...
  template<hashable T>
  inline constexpr auto hash(const T& value) -> hash_code {
    return static_cast<hash_code>(std::hash<T>{}(value));
  }

  inline constexpr auto hash_code_mix(
  ...
  inline constexpr auto hash_code_mix(const First& first, const Args&... rest)
    -> hash_code {
    return crab::hash_code_mix(first, crab::hash_code_mix(rest...));
  }

  template<hashable FirstItem>
  inline constexpr auto hash_together(const FirstItem& first) -> hash_code {
    return crab::hash(first);
  }

  template<hashable FirstItem, hashable... Items>
  inline constexpr auto hash_together(
