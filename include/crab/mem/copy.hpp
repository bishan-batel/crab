...
namespace crab::mem {
  ...
  auto copy(unsafe_fn, const T* source, T* destination, const usize count) -> void {
    ...
    crab_dbg_check(
      ...
      reinterpret_cast<uptr>(source) % alignof(T) == 0,
    ...
    crab_dbg_check(
      ...
      reinterpret_cast<uptr>(destination) % alignof(T) == 0,
    ...
    if constexpr (SIZE == 1) {
      memmove(static_cast<void*>(destination), static_cast<const void*>(source), count);
    } else {
      // check for integer overflow when multiplying
      //
      //  TODO: add back this check
      // crab_check(
      //   count == 0 or count > std::numeric_limits<usize>::max() / SIZE,
      //   "mem::copy integeroverflow converting count -> byte_count"
      // );

      const usize byte_count{count * mem::size_of<T>()};

      memmove(static_cast<void*>(destination), static_cast<const void*>(source), byte_count);
    }
  }
  ...
  auto copy_nonoverlapping(unsafe_fn, const T* source, T* destination, const usize count) -> void {
    ...
    {
      const uptr source_begin{reinterpret_cast<uptr>(mem::address_of(source[0]))};
      const uptr source_end{reinterpret_cast<uptr>(mem::address_of(source[count - 1]))};

      const uptr dest_begin{reinterpret_cast<uptr>(mem::address_of(destination[0]))};
      const uptr dest_end{reinterpret_cast<uptr>(mem::address_of(destination[count - 1]))};
    ...
    if constexpr (SIZE == 1) {
      memcpy(static_cast<void*>(destination), static_cast<const void*>(source), count);
    } else {
      ...
      memcpy(static_cast<void*>(destination), static_cast<const void*>(source), byte_count);
    }
  }
